...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    datatabel = $('#datatabel').DataTable({
      processing: true,
      serverSide : true,
      "ajax": "{{route('pasienDatagrid')}}",
      "columns": [
      { data:"nama_pasien", orderable:false, sortable:false, searchable:true},
      { data:"kd_pasien", orderable:false, searchable:true},
      { data:"nik_pasien", orderable:false, searchable:true},
      { data:"status_pernikahan", orderable:false, searchable:false},
      { data:"alamat_pasien", orderable:false, searchable:false},
      { data:"no_telepon", orderable:false, searchable:true},
  ...
  $("#btn-add").on('click', function() {
    ...
    $.post("{!! route('formpasien') !!}",{id:''}).done(function(data){
  ...
  function detailForm(id){
    ...
    $.post("{!! route('detailpasien') !!}",{id:id}).done(function(data){
      if (data.status == 'success') {
        $(".loading").hide();
        $('.other-page').html(data.content).fadeIn();
      }else{
  ...
  function editForm(id){
    ...
    $.post("{!! route('formpasien') !!}",{id:id}).done(function(data){
      console.log(data);
      if (data.status == 'success') {
        $(".loading").hide();
        $('.other-page').html(data.content).fadeIn();
  ...
  function deleteForm(id){
    ...
    function(){
      $.post("{!! route('deletepasien') !!}",{id:id}).done(function(data){
        if (data.status == 'success') {
