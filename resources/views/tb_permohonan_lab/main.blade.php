...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    datatabel = $('#datatabel').DataTable({
      processing: true,
      serverSide : true,
      "ajax": "{{route('tbpermohonanLabDatagrid')}}",
      "columns": [
  ...
  $("#btn-add").on('click', function() {
    ...
    $.post("{!! route('formpermohonanlab') !!}",{id:''}).done(function(data){
  ...
  function detailForm(id){
    ...
    $.post("{!! route('detailpermohonanlab') !!}",{id:id}).done(function(data){
      if (data.status == 'success') {
        $(".loading").hide();
        $('.other-page').html(data.content).fadeIn();
      }else{
  ...
  function editForm(id){
    ...
    $.post("{!! route('formpermohonanlab') !!}",{id:id}).done(function(data){
      console.log(data);
      if (data.status == 'success') {
        $(".loading").hide();
        $('.other-page').html(data.content).fadeIn();
  ...
  function deleteForm(id){
    ...
    function(){
      $.post("{!! route('deletepermohonanLab') !!}",{id:id}).done(function(data){
        if (data.status == 'success') {
